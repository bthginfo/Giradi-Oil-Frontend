...
export function OrderConfirmationPage() {
  ...
  // Try router state first, then sessionStorage fallback (page refresh)
  const stateFromRouter = location.state as {
    orderNumber?: string;
    orderId?: string;
    displayId?: number;
    payment?: string;
  ...
  const state = stateFromRouter || (() => {
    try {
      const stored = sessionStorage.getItem("tgo_last_order");
      if (!stored) return null;
      const order = JSON.parse(stored);
      return {
        orderNumber: order.display_id
          ? `TGO-${order.display_id}`
          : order.id
          ? `TGO-${order.id.slice(-8).toUpperCase()}`
          : undefined,
        orderId: order.id,
          ...
          <div className="bg-cream rounded-xl p-6 text-left space-y-4 mb-8">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {payment === "paypal" && <Mail className="w-4 h-4 text-olive-500" />}
